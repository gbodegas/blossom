...
def test_a_long_list_costs_no_more_reads_than_a_short_one(
    ...
    # however long either list is. The page of the list reads no notes. Both read the
    # school's instructions with the record, once.
    assert costs == ([10, 10, 10] if where == HER_PAGE else [9, 9, 9])
...
def test_a_result_for_an_assignment_off_the_record_is_said_only_when_its_history_bears_it_out(
    ...
    assert len(seen) == (10 if origin == "week" else 9)
